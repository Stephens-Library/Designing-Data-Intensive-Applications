...
1. As an application developer, you look at the real world and model it in terms of objects, data structures, and APIs that manipulate those data structures, those structures are often specific to your applicationm
...
3. The engineers who built your database software decided on a way of preresenting htat JSON/XML/relational/graph data in terms of bytes in memory, on disk, or on a network. The representation may allow the data to be queried, searched, maniupulated, and processed in various ways
...
There are different kinds of data models, and every data model emobdies assumptions about how it is going to be used, some kinds of usage are easy and some are not supported
...
Building software is hard enough, even when working with just one data model and without worrying about its inner workings, but since the data model has such a profoudn effect on what the software above it can and can't do, it's important to choose one that is appropriate to the application
